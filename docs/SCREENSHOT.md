...
1. `npm run gui` (or launch the installed app).
2. **Select repo…** → pick any TS/JS repo → **Run evaluation**.
3. When the dashboard loads, capture it:
   - **Windows:** `Win` + `Shift` + `S` (Snip & Sketch), drag over the window.
...
Tip: use a repo with no private code, or blur file paths, since the shot is public.
